...
*/
		...
		for (i = 0; i < size.height; i++) {
			for (j = 0 ; j < size.width; j++) {
				...
				uchar	*ptrd;
				uchar	*ptri;
				ptrm= (uchar*) (bg->imageData + i * bg->widthStep);
...
//				if ( ptrm[j] && (i >= comp_rect.y) && (j >= comp_rect.x) && (i < comp_rect.y + comp_rect.height) && (j < comp_rect.x + comp_rect.width) ) {
				...
				} else if ( (i >= comp_rect.y) && (j >= comp_rect.x) && (i < comp_rect.y + comp_rect.height) && (j < comp_rect.x + comp_rect.width) ) {
                                        //last_change = frame_num;
                                        //last_x = comp_rect.x;
					ptrd[3*j] = ptrm[3*j];
					ptrd[3*j+1] = ptrm[3*j +1];
					ptrd[3*j+2] = ptrm[3*j +2];
				} else {
					ptrd[3*j] = ptri[3*j];
					ptrd[3*j+1] = ptri[3*j+1];
					ptrd[3*j+2] = ptri[3*j+2];
