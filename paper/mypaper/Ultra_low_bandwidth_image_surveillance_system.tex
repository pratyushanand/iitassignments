...
\title{Ultra Low Bandwidth Image Surveillance System}
...
object based on these information.We have also analyzed feasibility of
...
significantly.Extraction of such image information is possible if end
node is intelligent. Semantic information transfer can reduce data rate
further.  But, we also need to ensure that this minimization of content
allows us to reconstruct the image with relevant details at the other
end.\\
\indent Automatic analysis of scene and then to infer desired
information out of it can be lot complex because of the complexity of
...
some other scene ambient light can be different. Similarly can be many
other hurdles in the process of automation. But if an efficient
...
latest research in computer vision, it seems that such task could be
...
selected object ontology has been used as the recognition method. Once
key information has been identified, they are sent to a server where
original scene is recreated.\\
...
feature for estimation. But only color as feature might not be
...
few surveillance objects such as human, animal, car etc and that too
with minimal computational cost.There have been several work which used
shape or skeleton based approach to detect moving object. Such
implementation seems to be of fairly low computational complexity.
Medial axis transform, distance transform and morphological
skeletonization etc are different methods which can be used to find out
skeleton of a moving object.\\
...
\ref{bg_compare} shows the comparison of execution time of these tow
algorithm.
