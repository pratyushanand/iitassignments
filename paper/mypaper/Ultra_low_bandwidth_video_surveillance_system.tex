...
% affiliations
\author{\IEEEauthorblockN{Pratyush Anand}
\IEEEauthorblockA{Indian Institute of Technology Delhi \\
Hauz Khas, New Delhi \\
email: eey107515@ee.iitd.ac.in}
\and
\IEEEauthorblockN{Prof. Subrat Kar}
\IEEEauthorblockA{Indian Institute of Technology Delhi \\
Hauz Khas, New Delhi \\
...
image pipeline.We are stripping down each moving object to few pixel,
mainly the skeleton endpoints and then also try to recognize moving
object based on motion information.We have also analyzed feasibility of
such a low data rate image pipeline for a low cost embedded system.
\end{abstract}
...
of person. But such an arrangement can not be full proof. So, any
...
and assembly line inspection etc.\\
...
researchers. \\ 
\indent ~\cite{1} and many other have used color information as the
feature for background estimation. But only color as feature might not be
sufficient ~\cite{2} in surveillance applications where image resolution
is very low.  Adding motion along with image intensities gives better
result in moving bodies estimation. When background color is same as
foreground color then texture features represented by local binary
pattern ~\cite{3} provides interestingly nice result. Further, features
computed at a single scale can be used to approximate feature at nearby
scale. Such implementations ~\cite{4} accelerates the execution for
practical applications.Methods based on Gaussian Mixtures Model(GMM)
have also been widely used, but they are computationally very expensive.
There is one non-parametric implementation ~\cite{5} known as Vibe, which
decides about background pixel value after comparison with closest samples
...
Commercial algorithms such as MPEG-4 also employees object based
coding.Applications which we have targeted in this work is to identify
few surveillance objects such as human, animal, vehicle etc and that too
with minimal computational cost.Shape or skeleton ~\cite{11} based based
approach seems computationally efficient for object recognition.  Medial
axis transform, distance transform, morphological and star
skeletonization etc ~\cite{9} ~\cite{10} are different methods which can
be used to find out skeleton of a moving object.\\
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.35]{image_pipeline}
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.35]{bg_compare}
...
We get several noise object in the extracted foreground image, see fig
\ref{pipeline_images}b. They are cleaned by morphological erosion
operation followed by dilation operation and we get fig
\ref{pipeline_images}c. Further all moving contours are separated out by
using opencv library ~\cite{12} function cvFindContours.This function
gives us boundary point of moving object as show in fig
\ref{pipeline_images}d.

\begin{figure}[!h]
\centering
\includegraphics[scale=0.30]{pipeline_images}
...
image \textbf{d.} Contour of moving object \textbf{e.} Plot of smoothed
distance of boundary points from centroid \textbf{f.} Plot of 3 points
...
point as follows.This calculated distance vetor are stored in a CvMat
array.
	\begin{equation}
	d_i = \sqrt{(C_x - X_i)^2 + (C_y - Y_i)^2}
	\end{equation}
\item Distance vetor array is smoothed to remove noise peaks using
cvSmooth.If these distances are ploted then it looked like fig
\ref{pipeline_images}e. Now local maxima of distance vector is
...
as shown in fig \ref{pipeline_images}f. Only these two peaks along with
centroid will give us sufficient information to distinguish an object
among human, vehicle or animal.\\
In case of human these two peaks depicts two legs.For vehicle these are
just two extream points of lower portion of back and front.In case of
...
respectively. Let $\theta$ is the angle beween P$_1$C and P$_2$C, then
$\theta$ can be calculated as follows.\\
%
	\begin{equation}
	\theta = atan2(Y_2 - C_y, X_2 - C_x) - atan2(Y_1 - C_y, X_1 - C_x)
	\end{equation}
%
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.35]{angle_plot}
...
stored. 
\begin{enumerate} 
\item We track value of $\theta$ untill it goes to '0' three times.
...
\item If n is the length of vector T$_1$ then caculate corelation value
...
- m_2)^2}}}
	\end{equation}
\item If corelation value is greater than a threshold value TH$_1$ then we say
that it is a human.
\item If we do not find any '0' till N frames, then we calculate varinace of
angle data for N frames. If it is less than threshold value TH$_2$, then
we say that it is a vehicle.
\item If variance is more than threshold value TH$_3$, then it is an
animal. 
